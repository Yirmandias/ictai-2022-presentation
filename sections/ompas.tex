\section{OMPAS : RAE guided by planning}
...
\begin{frame}{Improve high-level choices by anticipating RAE choices in function of the agent behavior}
    ...
    \begin{itemize}
        ...
        \pause
        \item informed choice that accounts for long terms impacts
        \pause
...
\begin{frame}{Input and output of the planner}
    ...
    \begin{tikzpicture}[thick,scale=0.8, every node/.style={scale=0.8}]
        \node[draw = black, very thick,
            ...
            rounded corners,
            text = black,
            align=center] (F) at (0,0) {\textbf{Planner}\\Aries};
        ...
        \node[xshift = 12em] (t5) {\textbf{Hierarchical Plan}};
...
\begin{frame}{Little summary}
    What has been presented so far:
    \begin{itemize}
        \item OMPAS: A new RAE implementation
        \item Select using a hierarchical temporal planner
    \end{itemize}

    Now:
    \begin{itemize}
        \item Acting language from which we can extract chronicles
        \item An example using planning
    \end{itemize}
\end{frame}
...
\begin{frame}{Extraction of descriptive models from SOMPAS}
    \begin{tikzpicture}
        ...
        \node[below=0em of ssa, align = center] {\scriptsize (lower level\\ \scriptsize formalism)};
...
\begin{frame}[fragile]{Example: the gripper-door problem}
    \begin{columns}[c]
        \begin{column}{0.3\textwidth}
            \centering
    
            \begin{overprint}
                \onslide<1>\includegraphics[width = 0.8\textwidth]{images/3_rooms/gd_3_0.png}
                \onslide<2>\includegraphics[width = 0.8\textwidth]{images/3_rooms/gd_3_1.png}
                \onslide<3>\includegraphics[width = 0.8\textwidth]{images/3_rooms/gd_3_3.png}
                \onslide<4->\includegraphics[width = 0.8\textwidth]{images/3_rooms/gd_3_6.png}
            \end{overprint}
        \end{column}
        \begin{column}<1->{0.7\textwidth}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    \item at-robby(robot) $\rightarrow$ loc
                    \item at(ball) $\rightarrow$ loc
                    \item carry(gripper) $\rightarrow$ ball
                    \item connected(loc, loc) $\rightarrow$ bool
                \end{itemize}
                \onslide<6->
                \item Tasks and methods:
                
                \begin{tikzpicture}
                    \node(t1) {\textit{pick-and-drop}};
                    \node[below = 1em of t1] (m) {m};
                    \node[below = 1em of m, xshift = -4em] (s1) {$t_{move}$};
                    \node[below = 1em of m, xshift = -1.5em] (s2) {\textbf{pick}};
                    \node[below = 1em of m, xshift = 1.5em] (s3) {$t_{move}$};
                    \node[below = 1em of m, xshift = 4em] (s4) {\textbf{drop}};
                    \path[-] (t1) edge (m)
                     (m) edge (s1)
                     (m) edge (s2)
                     (m) edge (s3)
                     (m) edge (s4);
                    \onslide<7->
                     \node[right = 6em of t1] (t2) {$t_{move}$};
                    \node[below = 1em of t2, xshift= -1.5em] (m1) {$m_{noop}$};
                    \node[below = 1em of t2, xshift = 1.5em] (m2) {$m_r$};
                    \node[below = 1em of m2, xshift = -1.5em] (s5) {\textbf{move}};
                    \node[below = 1em of m2, xshift = 1.5em] (s6) {$t_{move}$};
...
\begin{frame}[c,fragile]{Gripper-door: example of the extraction of the descriptive model of a method}
    \small
...
\begin{frame}[fragile]{Gripper-door: platform model in PDDL fashion}
    \small
    \begin{itemize}
        \item Defined with SOMPAS only for planning purpose
        \pause
        \item \textbf{assert} primitive to represent effects %$\rightarrow$ \textbf{setstate} $\rightarrow$ \textbf{effect}
        \pause
    \end{itemize}
...
\begin{frame}[c, fragile]{Benchmarking gripper-door: problems of different sizes}
    \begin{columns}[c]
...
\begin{frame}{Results for gripper-door}
